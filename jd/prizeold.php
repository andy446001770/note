...
$sr = $pdo->query("UPDATE prize SET probability=$probability WHERE prize_id=4");
/**
* 经典的概率算法，
* $proArr是一个预先设置的数组，
...
//print_r($res['yes']);
//exit;
$sql="SELECT prize,get_prize,is_share FROM weixin WHERE openid="."'$openid'";
$sr = $pdo->query($sql);
$row=$sr->fetchAll();
if(intval($row[0]['get_prize'])==1&&intval($row[0]['is_share'])==1) {
