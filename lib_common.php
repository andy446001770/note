...
{
    $url = empty($image) ? "images/no_picture.gif" : $image;
    return $url;
}
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
    ...
    $user_price    = $goods['shop_price'];
        $sql="SELECT levels FROM ecs_users WHERE user_id=$_SESSION[user_id]";
        $levels=$GLOBALS['db']->getOne($sql);
        $sql="SELECT * FROM ecs_goods WHERE goods_id=$goods_id";
        $price2=$GLOBALS['db']->getAll($sql);
        if(intval($levels)==1){
            $user_price  =  $price2[0]['ls_price'];
        }elseif(intval($levels)==2){
            $user_price  = $price2[0]['jd_price'];
        }elseif(intval($levels)==3){
            $user_price  = $price2[0]['xz_price'];
        }elseif(intval($levels)==4){
            $user_price  = $price2[0]['xj_price'];
        }elseif(intval($levels)==5){
            $user_price  = $price2[0]['by_price'];
        }
    //比较商品的促销价格，会员价格，优惠价格
